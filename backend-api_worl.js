...
// IMPORT DELL'ADATTATORE DENO/NODE CORRETTO
...
// Middleware CORS (per sbloccare il frontend!)
// NEL TUO file backend-api.js

// Middleware CORS (per sbloccare il frontend!)
app.use(cors({
  origin: "http://localhost:5173", // Permetti richieste SOLO dal tuo frontend
  methods: "GET, POST, PUT, DELETE, OPTIONS", // ESSENZIALE: Includi OPTIONS!
  allowedHeaders: "Content-Type, Authorization", // Permetti questi header
  optionsSuccessStatus: 200 // Rispondi 200 (OK) alle richieste preflight
}));

// Aggiungiamo un "catcher" manuale per OPTIONS, per sicurezza
app.options('*', cors()); // Rispondi a TUTTE le richieste OPTIONS con la config CORS
...
app.get('/health', (req, res) => {
  console.log("[DEBUG] RICHIESTA /health RICEVUTA!"); // Aggiungiamo un log
  res.json({ 
    status: 'ok', 
    identita_server: 'SONO IL SERVER DENO/EXPRESS CORRETTO V3' // <-- IL NOSTRO MARCHIO!
  });
});
...
app.get('/auth/me', requireDB, authenticateToken, async (req, res) => {
  try {
    const { data: user, error } = await supabase
      .from('users').select('id, email, full_name, role, created_date').eq('id', req.user.id).single();
    if (error) throw error;
    res.json(user);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
// USA IL NUOVO SERVER JSR
const server = new Server({
  handler: app, // Passa la tua app Express all'handler
  onError: (err) => {
    console.error("Errore del server Node:", err); // <-- Qui c'era l'errore di <eof>
...
// Avvia il server
server.listen(PORT, () => {
  console.log(`\n🚀 Backend API (Express su Deno) running on port ${PORT}`);
